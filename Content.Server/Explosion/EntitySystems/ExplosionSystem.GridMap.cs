...
public sealed partial class ExplosionSystem
{
    ...
    /// </remarks>
    private bool IsEdge(EntityUid gridUid, MapGridComponent grid, Vector2i index, out NeighborFlag spaceDirections)
    {
        spaceDirections = NeighborFlag.Invalid;
        for (var i = 0; i < NeighbourVectors.Length; i++)
        {
            if (!_map.TryGetTileRef(gridUid, grid, index + NeighbourVectors[i], out var neighborTile) || neighborTile.Tile.IsEmpty)
