    ...
    public partial class PipeNode : Node, IGasMixtureHolder, IRotatableNode
    {
            ...
            IEntityManager entMan)
        {
            ...
            for (var i = 0; i < PipeDirectionHelpers.PipeDirections; i++)
            {
                ...
                foreach (var pipe in LinkableNodesInDirection(pos, pipeDir, gridUid, grid, map, nodeQuery))
                {
                    yield return pipe;
                }
            }
        ...
        /// </summary>
        private IEnumerable<PipeNode> LinkableNodesInDirection(Vector2i pos, PipeDirection pipeDir, EntityUid gridUid, MapGridComponent grid,
            SharedMapSystem map, EntityQuery<NodeContainerComponent> nodeQuery)
        {
            foreach (var pipe in PipesInDirection(pos, pipeDir, gridUid, grid, map, nodeQuery))
            {
        ...
        /// </summary>
        protected IEnumerable<PipeNode> PipesInDirection(Vector2i pos, PipeDirection pipeDir, EntityUid gridUid, MapGridComponent grid,
            SharedMapSystem map, EntityQuery<NodeContainerComponent> nodeQuery)
        {
            var offsetPos = pos.Offset(pipeDir.ToDirection());

            foreach (var entity in map.GetAnchoredEntities(gridUid, grid, offsetPos))
            {
